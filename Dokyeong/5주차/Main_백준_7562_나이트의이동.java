package al_5주차;

import java.awt.Point;
import java.util.LinkedList;
import java.util.Queue;
import java.util.Scanner;
/*문제 요약
 * 체스 움직일 수 있는 곳 정해짐
 * 시작점-도착점 이동 최소 횟수
 * 
 * 아이디어 
 * BFS+Queue
 * n방 탐색 
 * 목적지 도달 최소값 - bfs
 * while 저장, 큐 level 탐색
 */
public class Main_백준_7562_나이트의이동 {
	static int[] dr = { -2, -1, 1, 2, 2, 1, -1, -2 };
	static int[] dc = { 1, 2, 2, 1, -1, -2, -2, -1 }; // n방 탐색 

	static int T, I, answer;
	static int[][] map;

	public static void main(String[] args) {
		Scanner sc = new Scanner(System.in);
		Queue<Point> queue = new LinkedList<>();

		T = sc.nextInt();

		for (int test_case = 1; test_case <= T; test_case++) {
			I = sc.nextInt(); // 체스 범위
 
			map = new int[I][I]; // 체스 크기 
			Point start = new Point(sc.nextInt(), sc.nextInt()); // 시작지점
			Point end = new Point(sc.nextInt(), sc.nextInt()); // 도착지점

			map[start.x][start.y] = 1; // 방문 표시
			queue.add(new Point(start.x, start.y));

			L: while (true) {
				int queuesize = queue.size();
				if (queuesize == 0)
					break;
				int n = 0;
				while (n++ < queuesize) {
					Point Cur = queue.poll();

					if (Cur.x == end.x && Cur.y == end.y)
						break L; // 목적지 도착

					for (int d = 0; d < 8; d++) {
						int nr = Cur.x + dr[d];
						int nc = Cur.y + dc[d];

						if (nr < 0 || nr >= I || nc < 0 || nc >= I || map[nr][nc] == 1)
							continue; // boundary check , 이미 방문한 곳이면

						map[nr][nc] = 1; // 방문처리 후에
						queue.add(new Point(nr, nc)); // 큐에 넣자

					}

				}
				answer++;
			}

			System.out.println(answer);
			answer = 0;
			queue.clear();
		}
	}// end of main
}// end of class

/* 관흠 리뷰
 * 
 *  저는 나이트가 이동하는 횟수를 따로 저장하는 클래스를 만들어서 관리해 줬는데
 *  매 횟수마다의 큐사이즈를 통해 반복문을 진행하셨군요
 *  참신합니다!
 * 
 * */
